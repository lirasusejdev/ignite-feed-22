...
const useScroll = ref => {
  const [clickStartX, setClickStartX] = (0, _react.useState)();
  const [scrollStartX, setScrollStartX] = (0, _react.useState)();
  const [isDragging, setIsDragging] = (0, _react.useState)(false);
  const [direction, setDirection] = (0, _react.useState)(0);
  const [momentum, setMomentum] = (0, _react.useState)(0);
  const [lastScrollX, setLastScrollX] = (0, _react.useState)(0);
  const [speed, setSpeed] = (0, _react.useState)(0);
  const [reachedStart, setReachedStart] = (0, _react.useState)(false);
  const scrollWrapperCurrent = ref.current;
  const handleLastScrollX = (0, _react.useCallback)(x => setLastScrollX(x), []);
  const handleMomentum = (0, _helpers.throttle)(nextMomentum => {
