...
const preventDefault = rawEvent => {
  const e = rawEvent || window.event; // Do not prevent if the event has more than one touch (usually meaning this is a multi touch gesture like pinch to zoom).

  if (e.touches.length > 1) return true; // $FlowFixMe[method-unbinding]
...
const setPositionFixed = () => window.requestAnimationFrame(() => {
  ...
  if (previousBodyPosition === undefined) {
    ...
    document.body.style.left = `${-scrollX}px`; // avoid a bug on iOS Safari where body doesn't take up full width
...
const restorePositionSetting = () => {
  ...
  if (previousBodyPosition) {
    const {
      top,
      left
    } = document.body.style; // Restore styles
    ...
    const x = -parseInt(left, 10); // Restore scroll
...
const isTargetElementTotallyScrolled = targetElement => !!targetElement && targetElement.scrollHeight - targetElement.scrollTop <= targetElement.clientHeight;
